...
YARN[1.21.6]="1.21.9+build.1"
YARN[1.21.7]="1.21.10+build.1"
YARN[1.21.8]="1.21.11+build.1"
...
LOADER[1.21.9]="0.16.14"
LOADER[1.21.10]="0.16.14"
LOADER[1.21.11]="0.16.14"
...
for V in "${MC_VERSIONS[@]}"; do
    ...
    if [[ "$V" == "1.20" || "$V" == "1.20.1" ]]; then
        rm -rf src
        cp -r src2 src
    elif [[ "$V" == "1.21.6" || "$V" == "1.21.7" || "$V" == "1.21.8" || "$V" == "1.21.9" || "$V" == "1.21.10" || "$V" == "1.21.11" ]]; then
        rm -rf src
        cp -r src4 src
    else
        rm -rf src
